...
(fn :ret route-matches? ((route : list?) (path : string?) -> boolean?)
    (let ((route-segments (vector->list (car route)))
      ...
      (call/cc
        (lambda (return)
          ...
          (fn :ret match-segment ((seg1 : segment?) (seg2 : segment?) -> boolean?)
...
(fn :ret router ((static : [symbol? procedure?])
                 (recovery : [symbol? procedure?])
                 (routes : (list-of vector?))
                 -> procedure?)

    (fn :r subset? ((list1 : list?) (list2 : list?) -> boolean?)
        (every
         (lambda (x)
           (member x list2))
         list1))

    (fn :r default-static-handler ((path : string?) -> (or-false? file-path?))
        (printf "static: ~a\n" path)
        (sanitize-static-path path))

    (fn :r default-recovery ((rejection : rejection?) -> string?)
        (error-template
      ...
      (lambda (req res)
        (let ((path     (http-request-path    req))
          ...
          (let* ((matching-routes      (filter (lambda (route)
            ...
            (fn :r exec-handlers (-> resolution?)
                (call/cc
                  (lambda (resolve)
                    (fn :rr iterate ((handler-pair : pair?) -> resolution?)
